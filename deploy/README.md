...
HTTP/2 включён параметром `listen 443 ssl http2;`, а не отдельной директивой `http2 on;`:
вторая появилась в nginx 1.25.1 и на более старом сервере валит конфиг. Параметр `listen`
понимают обе версии, в новых он помечен устаревшим, но работает.
...
`nginx -t` и перезагружает nginx. При ошибке проверки шаг возвращает прежний конфиг
на место и падает — иначе на диске остался бы нерабочий файл, и следующая перезагрузка
nginx уронила бы вход целиком. Правки маршрутов preprod доезжают
